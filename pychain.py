...
input_value_receiver = st.text_input("Enter Address of Receiver")
if st.button("Submit Receiver Address"):
    st.write(input_value_receiver)

# @TODO:
# Add an input area where you can get a value for `amount` from the user.
input_value_amount = st.text_input("Enter Amount")
if st.button("Submit Amount"):
    st.write(input_value_amount)
...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_block = Block(
        record=Record(input_value_sender, input_value_receiver, input_value_amount),
        creator_id=42,
